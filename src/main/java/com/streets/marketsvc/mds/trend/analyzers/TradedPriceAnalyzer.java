...
@Service
public class TradedPriceAnalyzer implements Analyzer{
    @Override
    public TrendResult analyze(List<RawExchangeData> data) {
        List<Double> movingAverages = Calculator.movingAverage(data.stream().mapToDouble(RawExchangeData::getLastTradedPrice).boxed().collect(Collectors.toList()), 2);
        ...
        tr.setValueType("LAST_TRADED_PRICE");
