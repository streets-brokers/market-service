...
@Service
public class BidPriceAnalyzer implements Analyzer{
    @Override
    public TrendResult analyze(List<RawExchangeData> data) {
        List<Double> movingAverages = Calculator.movingAverage(data.stream().mapToDouble(RawExchangeData::getBidPrice).boxed().collect(Collectors.toList()), 2);
